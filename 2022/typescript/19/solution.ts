...
const runBlueprint = (
  ...
  if (robotsOre >= maxOreCost) robotsOre = maxOreCost;
  ...
  const clayCap =
    minutes * blueprint.obsidianRobotCost.clay - robotsClay * (minutes - 1);
  const obsidianCap =
    minutes * blueprint.geodeRobotCost.obsidian -
    robotsObsidian * (minutes - 1);

  if (ore >= oreCap) ore = oreCap;

  if (clay >= clayCap) clay = clayCap;

  if (obsidian >= obsidianCap) obsidian = obsidianCap;
...
runPart("One", () =>
  blueprints
    .map((b) => runBlueprint(b, 24, 0, 0, 0, 0, 1, 0, 0, 0) * b.id)
    .reduce((a, b) => a + b, 0)
); // Part One: 960 took 8664ms
runPart("Two", () =>
  blueprints
    .slice(0, 3)
    .map((b) => runBlueprint(b, 32, 0, 0, 0, 0, 1, 0, 0, 0))
    .reduce((a, b) => a * b, 1)
); // Part One: 2160 took 17897ms;
