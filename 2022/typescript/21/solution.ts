...
fs.readFileSync("./input.txt", "utf-8")
  ...
  .forEach((line) => {
    ...
    if (/\d+/.test(expr)) lookup[monkey] = Number(expr);
    else {
      expr = expr.split(" ");
...
const findHumanValue = () => {
  ...
  const humanBranch: "left" | "right" = !probeLeft.every(
    (v) => v === probeLeft[0]
  )
    ? rootBranches.shift()
    : !probeRight.every((v) => v === probeRight[0])
    ? rootBranches.pop()
    : undefined;
