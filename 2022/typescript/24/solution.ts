...
const shortestSafePath = (
  initialBlizzards: typeof Blizzards,
  startPos: Vec,
  targetPos: Vec
): {
  time: number;
  ...
  while (queue.length > 0) {
    ...
    if (visited.has({ pos, time })) continue;
    ...
    for (const dir of Object.values(Directions)) {
      ...
      if (Walls.has(newPos)) continue;
      if (pos[0] < minWallCol || pos[0] > maxWallCol) continue; // If we're at start or target we don't want to go look outside the map
...
runPart("One", () => {
  startToTarget = shortestSafePath(
    Blizzards,
    [0, 1],
    [maxWallCol, maxWallRow - 1]
  );
  return startToTarget.time;
}); // 225 took 2836ms

runPart("Two", () => {
  backToStart = shortestSafePath(
    startToTarget.blizzardsConfig,
    [maxWallCol, maxWallRow - 1],
    [0, 1]
  );
  backToTarget = shortestSafePath(
    backToStart.blizzardsConfig,
    [0, 1],
    [maxWallCol, maxWallRow - 1]
  );
