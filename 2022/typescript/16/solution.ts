...
const valveDefs: {
  [key: string]: { numId: number; flowRate: number; connections: string[] };
} = {};
...
Object.entries(valveDefs).forEach(([valveLabel, valve]) => {
  ...
  Tunnels[valve.numId] = valve.connections.map(
    (connection) => valveDefs[connection].numId
  );
...
const maxRelief = (
  ...
  if (minutesLeft <= 0) {
    return type === "P1" ? 0 : maxRelief(ValveIds["AA"], opened, 26, "P1");
  }

  const cacheKey = `${valve},${minutesLeft},` + opened;
  if (cache[type].has(cacheKey)) return cache[type].get(cacheKey)!;
  ...
  if (opened[valve] === "0" && Flowrates[valve] > 0) {
    ...
    Tunnels[valve].forEach((nextValve) => {
      const reliefed =
        valveRelief + maxRelief(nextValve, newOpened, minutesLeft - 2, type);
...
runPart("One", () =>
  maxRelief(ValveIds["AA"], "0".repeat(Object.keys(ValveIds).length), 30, "P1")
); // 1460 took 1299ms
runPart("Two", () =>
  maxRelief(ValveIds["AA"], "0".repeat(Object.keys(ValveIds).length), 26, "P2")
); // 2117 took 57707ms
