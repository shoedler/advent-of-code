...
const caveScan = fs
  ...
  .split("\r\n")
  .map((scanLine) =>
    scanLine.split(" -> ").map((coord) => coord.split(",").map(Number))
  ) as Vec[][];
...
caveScan.forEach((lineScan) => {
  for (let i = 0; i < lineScan.length - 1; i++) {
    ...
    if (dX === 0) {
      for (let y = y1; y <= y2; y++) caveWalls.add(`${y},${prevX}`);
    } else {
      for (let x = x1; x <= x2; x++) caveWalls.add(`${prevY},${x}`);
    }
  }
});
...
const dropOnce = (type: "P1" | "P2") => {
  ...
  while (true) {
    if (below()) [y, x] = [y + 1, x];
    else if (belowLeft()) [y, x] = [y + 1, x - 1];
    else if (belowRight()) [y, x] = [y + 1, x + 1];
    else {
      if (type === "P1") {
        if (y >= maxY)
          // Fell into the abyss
        ...
        grainsOfSand.add(`${y},${x}`);
        if (equals([y, x], sandSource)) return false;
        return true;
      }
    }
  }
